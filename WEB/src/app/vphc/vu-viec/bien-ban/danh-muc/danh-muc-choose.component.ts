...
export class DanhMucBienBanComponent implements OnInit {
  ...
  save(isContinue: boolean = false) {
    if (!this.currentRowData) {
      this.messageService.showWarning(
        'Hãy lựa chọn một mẫu để tiến hành lập biên bản.'
      );
      return;
    }
  ...
  getDanhMuc() {
    this.bienBanService.getDanhMuc().subscribe({
      next: (result) => {
        if (result.isStatus) {
          this.listDanhMuc = result.data;
          this.grDanhMuc.selectedRowIndex = 0;
        }
      },
      error: (error) => {
