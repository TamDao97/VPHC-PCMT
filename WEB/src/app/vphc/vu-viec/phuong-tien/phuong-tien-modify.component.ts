...
@Component({
  selector: 'app-phuong-tien-modify',
  templateUrl: './phuong-tien-modify.component.html',
  styleUrls: ['./phuong-tien-modify.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class PhuongTienModifyComponent implements OnInit {
  constructor(
  ...
  ngOnInit(): void {
    if (!this.id) {
      this.model.idPhuongTienVPHC = uuidv4();
    }else{
      this.isEdit = true;
    }
  ...
  public listLoaiPhuongTien: any[];
  public listDonViTinh: any[];
  getDataCombobox() {
    this.comboboxService.getLoaiPhuongTien().subscribe({
      next: (result) => {
        if (result.isStatus) {
          this.listLoaiPhuongTien = result.data;
        }
      },
      error: (error) => {
  ...
  onSelect(event: any, type: any): void {
    if (type == 'PhuongTien') {
      this.model.tenLoaiPhuongTien = event.itemData.name;
      if(event.itemData.id === 'FixOther')
      {
        this.model.tenPhuongTien =  this.model.name;
      }
    } 
  }
  //#endregion
