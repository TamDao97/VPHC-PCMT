...
@Component({
  selector: 'app-tang-vat-modify',
  templateUrl: './tang-vat-modify.component.html',
  styleUrls: ['./tang-vat-modify.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class TangVatModifyComponent implements OnInit {
  constructor(
  ...
  ngOnInit(): void {
    if (!this.id) {
      this.model.idTangVatVPHC = uuidv4();
    }else{
this.isEdit = true;
    }
  ...
  public listLoaiTangVat: any[];
  public listDonViTinh: any[];
  getDataCombobox() {
    this.comboboxService.getLoaiTangVat().subscribe({
      next: (result) => {
        if (result.isStatus) {
          this.listLoaiTangVat = result.data;
        }
      },
      error: (error) => {
  ...
  getDonViTinh(id: any) {
    this.comboboxService.getDonViTinh(id).subscribe({
      next: (result) => {
        if (result.isStatus) {
          this.listDonViTinh = result.data;
        }
      },
      error: (error) => {
        this.messageService.showError(error);
  ...
  onSelect(event: any, type: any): void {
    if (type == 'TangVat') {
      this.model.tenLoaiTangVat = event.itemData.name;
      this.getDonViTinh(event.itemData.id);
      if(event.itemData.id === 'FixOther')
      {
        this.model.tenTangVat =  this.model.name;
      }
    } else if (type == 'DonViTinh') {
      this.model.tenDonViTinh = event.itemData.name;
    }
  }
  //#endregion
